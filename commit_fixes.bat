@echo off
cd /d C:\Users\USUARIO\portal-lusitano

echo === Commit 1: next.config.js ===
git add next.config.js
git commit -m "fix(docker): add output standalone to next.config.js
...
if %ERRORLEVEL% EQU 0 (
    echo ✓ Commit 1 SUCCESS - SHA: 
    git rev-parse HEAD
) else (
    echo ✗ Commit 1 FAILED with exit code %ERRORLEVEL%
)

echo.
echo === Commit 2: api-factory-middleware.ts ===
git add lib/api-factory-middleware.ts
git commit -m "fix(security): implement real rate limiting with Upstash Redis
...
Co-authored-by: GitHub Copilot <copilot@github.com>"
if %ERRORLEVEL% EQU 0 (
    echo ✓ Commit 2 SUCCESS - SHA:
    git rev-parse HEAD
) else (
    echo ✗ Commit 2 FAILED with exit code %ERRORLEVEL%
)

echo.
echo === Commit 3: auth.ts + env.ts ===
git add lib/auth.ts lib/env.ts
git commit -m "fix(security): replace SHA-256 with scrypt for admin password hashing
...
Co-authored-by: GitHub Copilot <copilot@github.com>"
if %ERRORLEVEL% EQU 0 (
    echo ✓ Commit 3 SUCCESS - SHA:
    git rev-parse HEAD
) else (
    echo ✗ Commit 3 FAILED with exit code %ERRORLEVEL%
)

echo.
echo === Commit 4: ci.yml ===
git add .github/workflows/ci.yml
git commit -m "fix(ci): remove continue-on-error from production deploy job
...
Co-authored-by: GitHub Copilot <copilot@github.com>"
if %ERRORLEVEL% EQU 0 (
    echo ✓ Commit 4 SUCCESS - SHA:
    git rev-parse HEAD
) else (
    echo ✗ Commit 4 FAILED with exit code %ERRORLEVEL%
)
