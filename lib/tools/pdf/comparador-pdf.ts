...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function addLandscapeFooter(doc: any, language?: string): void {
    ...
    doc.text(
      safe(
        L(
          ...
          "portal-lusitano.pt  |  Analysis generated by Portal Lusitano. Estimated valuation — does not replace professional assessment.",
          "portal-lusitano.pt  |  Análisis generado por Portal Lusitano. Valoración estimativa — no sustituye evaluación profesional."
        )
      ),
      MARGIN,
      L_PAGE_H - 4.5
...
export async function generateComparadorPDF(
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  fillLandscapeBg(doc);
  addLandscapeTopBar(doc, L("Comparação de Cavalos", "Horse Comparison", "Comparación de Caballos"), date);
  ...
  cavalos.slice(0, n).forEach((cavalo, i) => {
    ...
    // Badge row
    if (isVencedor) {
      const bLabel = safe(L("Melhor Score", "Best Score", "Mejor Score"));
      doc.setFontSize(6);
      doc.setFont("helvetica", "bold");
      ...
      const bLabel = safe(L("Melhor Valor", "Best Value", "Mejor Valor"));
      doc.setFontSize(6);
      doc.setFont("helvetica", "bold");
      const bW = doc.getTextWidth(bLabel) + 6;
    ...
    doc.setFontSize(6);
    doc.setFont("helvetica", "normal");
    doc.text(L("Score Global", "Overall Score", "Score Global"), arcCX, y, { align: "center" });
    y += 5;
    ...
    const barItems = [
      { label: L("Conformação", "Conformation", "Conformación"), value: cavalo.conformacao, max: 10 },
      { label: L("Andamentos", "Gaits", "Aires"), value: cavalo.andamentos, max: 10 },
      { label: L("Saúde", "Health", "Salud"), value: cavalo.saude, max: 10 },
      { label: L("Temperamento", "Temperament", "Temperamento"), value: cavalo.temperamento, max: 10 },
    ];
  ...
  doc.addPage();
  fillLandscapeBg(doc);
  addLandscapeTopBar(doc, L("Tabela Comparativa", "Comparative Table", "Tabla Comparativa"), `Ref: ${refNum}`);
  ...
  const tableRows: { label: string; getValue: (c: CavaloComp, idx: number) => string }[] = [
    ...
    { label: L("Regularidade", "Regularity", "Regularidad"), getValue: (c) => `${c.regularidade}/10` },
