...
interface ProfissionaisContentProps {
  ...
  initialArtigos: ArtigoEducativo[];
}
...
export default function ProfissionaisContent({
  ...
  const [profissionaisBD, setProfissionaisBD] = useState<Profissional[]>(initialProfissionais);
  const [eventos, setEventos] = useState<Evento[]>(initialEventos);
  const [artigos, setArtigos] = useState<ArtigoEducativo[]>(initialArtigos);
  const [showEventoForm, setShowEventoForm] = useState(false);
  const [showArtigoForm, setShowArtigoForm] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Retry / refetch from API route (used by the error retry button)
  ...
  // Refetch functions for use after form submissions
  const fetchEventos = () => {
      ...
      <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
        ...
        <ParallaxSection speed={0.4} className="absolute inset-0 z-0">
          <Image
            ...
            alt="Cavalo lusitano em liberdade — directorio de profissionais equestres"
            fill
            sizes="100vw"
            ...
            <RevealOnScroll variant="fade-up">
              ...
              {eventos.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-[var(--foreground-muted)]">
                    {isProfissional
                      ? "Crie o primeiro evento para divulgar as suas clinicas e workshops."
          ...
          <RevealOnScroll variant="fade-scale">
            <div className="bg-gradient-to-r from-[var(--gold)]/10 to-transparent border border-[var(--gold)]/20 rounded-2xl p-8 sm:p-12 backdrop-blur-sm text-center">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[var(--gold)]/20 bg-[var(--gold)]/5 mb-8">
                ...
                <span className="text-sm font-bold text-gradient-gold">&euro;6/mes</span>
              </div>
