...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? t.login.logging_in : t.login.enter}
    </button>
  );
}
        ...
        <div className="relative z-20 w-full max-w-md bg-zinc-900/40 backdrop-blur-md border border-white/5 p-10 md:p-14 shadow-2xl">
            ...
            <form action={formAction} className="space-y-6">
                ...
                <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold">{t.login.email}</label>
                    <input
                      name="email"
                      type="email"
                      required
                ...
                <div className="space-y-2">
                    <div className="flex justify-between">
                        <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold">{t.login.password}</label>
                        <a href="#" className="text-[9px] uppercase tracking-widest text-zinc-600 hover:text-[#C5A059] transition-colors">{t.login.recover}</a>
                    </div>
                    <input
                      name="password"
                      type="password"
            ...
            <div className="mt-12 text-center">
                <p className="text-zinc-600 text-[10px] uppercase tracking-widest">
                    {t.login.no_invite} <Link href="/registar" className="text-white hover:text-[#C5A059] underline underline-offset-4 decoration-zinc-800 transition-colors">{t.login.create}</Link>
                </p>
            </div>
