...
import {
  ...
  Ban, Scale, ExternalLink, Droplets, Clock, FlaskConical,
  HeartPulse, Skull, TrendingUp, Info, CircleDot, Plane,
  ShieldAlert, CheckCircle2, XCircle, Flag, Microscope
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center flex-shrink-0">
        ...
        >
          <Icone size={16} className={`transition-colors duration-150 ${aberta ? 'text-black' : 'text-zinc-500 group-hover:text-[#C5A059]'}`} />
          {primeiraFechada && (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h2 className={`text-[15px] sm:text-xl font-serif leading-tight transition-colors duration-150 ${aberta ? 'text-[#C5A059]' : 'text-zinc-200 group-hover:text-white'}`}>
                {seccao.titulo}
              </h2>
              <p className={`text-[11px] sm:text-[13px] mt-1 transition-colors duration-150 ${aberta ? 'text-zinc-400' : 'text-zinc-600 group-hover:text-zinc-500'}`}>
                {seccao.subtitulo}
              </p>
              {!aberta && (
                <span className={`inline-flex items-center gap-1.5 mt-2.5 text-[11px] sm:text-xs font-medium transition-colors duration-150 ${primeiraFechada ? 'text-[#C5A059]' : 'text-zinc-600 group-hover:text-[#C5A059]'}`}>
                  Toque para ler
                  <span className={primeiraFechada ? "inline-block animate-[gentle-bounce_1s_ease-in-out_infinite]" : ""}>
                    <ChevronDown size={12} />
                  </span>
                </span>
              )}
...
function ContadorAnimado({ valor, cor }: { valor: string; cor: string }) {
  ...
  if (!match) return <div ref={ref} className={`text-xl sm:text-2xl font-bold ${cor}`}>{valor}</div>;
...
function DadoDestaque({ icone: Icone, valor, label, cor = "text-[#C5A059]" }: { icone: React.ElementType; valor: string; label: string; cor?: string }) {
  return (
...
function EstudoCard({ regiao, tEqui, bCaballi, fonte, metodo }: { regiao: string; tEqui: string; bCaballi: string; fonte: string; metodo: string }) {
  return (
    <div className="bg-zinc-800/40 rounded-xl p-4 sm:p-5 border border-zinc-700/30 transition-all duration-200 hover:scale-[1.02] hover:border-[#C5A059]/25">
      <div className="flex items-start justify-between mb-4">
        ...
        <span className="text-[9px] bg-zinc-700/50 text-zinc-400 px-2 py-0.5 rounded-full">{metodo}</span>
      </div>
      ...
      conteudo: (
        <div className="space-y-6 text-sm text-zinc-300 leading-relaxed">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <DadoDestaque icone={Globe} valor="~90%" label="da população equina vive em zonas endémicas" />
            <DadoDestaque icone={Skull} valor="5-10%" label="mortalidade em zonas endémicas" cor="text-red-400" />
            <DadoDestaque icone={AlertTriangle} valor=">50%" label="mortalidade em animais sem exposição prévia" cor="text-red-500" />
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div className="bg-red-500/5 border border-red-500/15 rounded-xl p-4 sm:p-5">
              ...
              <ul className="space-y-2 text-xs text-zinc-400">
                ...
                <li className="flex items-start gap-2">
                  <CircleDot size={10} className="text-red-400 mt-1 flex-shrink-0" />
                  <span>Cavalos infectados tornam-se <strong className="text-white">portadores crónicos persistentes</strong></span>
                </li>
                <li className="flex items-start gap-2">
                  <CircleDot size={10} className="text-red-400 mt-1 flex-shrink-0" />
            ...
            <div className="bg-orange-500/5 border border-orange-500/15 rounded-xl p-4 sm:p-5">
              ...
              <ul className="space-y-2 text-xs text-zinc-400">
                ...
                <li className="flex items-start gap-2">
                  <CircleDot size={10} className="text-orange-400 mt-1 flex-shrink-0" />
                  <span>Infecções podem ser <strong className="text-white">eliminadas naturalmente</strong> pelo organismo</span>
                </li>
                <li className="flex items-start gap-2">
                  <CircleDot size={10} className="text-orange-400 mt-1 flex-shrink-0" />
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                { label: "Carraças Ixodídeas", detalhe: "~14 espécies dos géneros Dermacentor, Rhipicephalus e Hyalomma", principal: true },
                { label: "Sangue contaminado", detalhe: "Produtos sanguíneos, transfusões", principal: false },
                { label: "Agulhas e seringas", detalhe: "Reutilização de material", principal: false },
                { label: "Equipamento IV", detalhe: "Material intravenoso partilhado", principal: false },
                { label: "Transplacentária", detalhe: "Égua para poldro", principal: false },
              ].map((via, i) => (
                <div key={i} className={`rounded-xl p-3 sm:p-4 border ${via.principal ? 'bg-[#C5A059]/10 border-[#C5A059]/20 col-span-2 sm:col-span-1' : 'bg-zinc-800/40 border-zinc-700/30'}`}>
                  <p className={`text-xs font-semibold mb-1 ${via.principal ? 'text-[#C5A059]' : 'text-white'}`}>{via.label}</p>
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                { nome: "Aguda", freq: "mais comum", cor: "border-red-500/30 bg-red-500/5", corTexto: "text-red-400", sintomas: ["Febre > 40°C", "Anemia", "Perda de apetite", "Taquicardia"] },
                { nome: "Subaguda", freq: "intermédia", cor: "border-orange-500/30 bg-orange-500/5", corTexto: "text-orange-400", sintomas: ["Perda de peso", "Febre intermitente", "Icterícia"] },
                { nome: "Crónica", freq: "comum em endémicos", cor: "border-yellow-500/30 bg-yellow-500/5", corTexto: "text-yellow-400", sintomas: ["Inapetência ligeira", "Baixo rendimento", "Perda de peso gradual"] },
                { nome: "Peraguda", freq: "rara", cor: "border-red-700/30 bg-red-700/5", corTexto: "text-red-600", sintomas: ["Falência multiorgânica", "Morte súbita"] },
              ].map((forma, i) => (
                <div key={i} className={`rounded-xl p-4 border ${forma.cor} flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6`}>
                  <div className="flex items-center gap-3 sm:w-44 flex-shrink-0">
                    <span className={`text-sm font-bold ${forma.corTexto}`}>{forma.nome}</span>
                    <span className="text-[9px] bg-zinc-800 text-zinc-500 px-2 py-0.5 rounded-full">{forma.freq}</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {forma.sintomas.map((s, j) => (
                      <span key={j} className="text-[11px] text-zinc-400 bg-zinc-800/60 px-2.5 py-1 rounded-lg">{s}</span>
                    ))}
                  </div>
                </div>
              ))}
      ...
      conteudo: (
        <div className="space-y-6 text-sm text-zinc-300 leading-relaxed">
          <p className="text-[15px] leading-relaxed">
            ...
            estão amplamente estabelecidas. A maioria dos cavalos nascidos em pastagens portuguesas
            é exposta ao parasita enquanto jovem, desenvolvendo anticorpos.
          </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-400 leading-relaxed">
                  ...
                  na seroprevalência. A co-infecção por ambos os parasitas foi registada em <strong className="text-white">7.4%</strong> dos casos analisados.
                </p>
              </div>
            </div>
          </div>
      ...
      conteudo: (
        <div className="space-y-6 text-sm text-zinc-300 leading-relaxed">
          ...
          <p className="text-[15px] leading-relaxed">
            ...
            de cavalos</strong> a nível mundial. Vários países exigem testes negativos
            para <em className="text-[#C5A059]">T. equi</em> e <em className="text-[#C5A059]">B. caballi</em> antes de permitirem a entrada.
          </p>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                ...
                <div key={pais} className="bg-red-500/5 border border-red-500/15 rounded-lg px-3 py-2.5 flex items-center gap-2">
            ...
            <p className="text-[10px] text-zinc-500 mt-2 ml-1">
              <strong className="text-zinc-400">Nota:</strong> China, Tailândia e México também exigem testes negativos
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {[
                { icone: Microscope, titulo: "Testes utilizados", valor: "IFAT, cELISA e CFT", detalhe: "Imunofluorescência, ELISA competitivo, fixação do complemento" },
                { icone: TrendingUp, titulo: "Limiar de negatividade", valor: "< 30%", detalhe: "Cavalos devem pontuar abaixo de 30% em todos os testes" },
                { icone: Clock, titulo: "Tempo de resultado", valor: "2 a 5 dias", detalhe: "Após colheita da amostra sanguínea" },
                { icone: Scale, titulo: "Custo", valor: "~60€ a 300€", detalhe: "~60€ em Portugal; 200-300€ em laboratórios como Bose (Alemanha)" },
              ].map((item, i) => (
          ...
          <div>
            ...
            <div className="bg-red-500/5 border border-red-500/15 rounded-xl p-5 sm:p-6">
              <p className="text-[13px] text-zinc-400 mb-4">
                ...
                Se um cavalo testar positivo após a importação:
              </p>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div className="bg-zinc-800/40 border border-zinc-700/30 rounded-xl p-4 sm:p-5">
              ...
              <p className="text-xs text-zinc-400 leading-relaxed">
                O stress do transporte pode causar a <strong className="text-white">subida dos títulos sanguíneos</strong> acima do limiar de 30%,
                fazendo com que um cavalo que testou negativo antes da partida possa testar positivo à chegada.
              </p>
            </div>
            <div className="bg-[#C5A059]/8 border border-[#C5A059]/20 rounded-xl p-4 sm:p-5">
              ...
              <p className="text-xs text-zinc-400 leading-relaxed">
                Criadores vendem cada vez mais cavalos &quot;piro-free&quot; como <strong className="text-white">categoria premium</strong>,
                criando duas categorias de valor comercial dentro da raça.
              </p>
            </div>
          </div>
        ...
        <div className="space-y-6 text-sm text-zinc-300 leading-relaxed">
          ...
          <div className="space-y-4">
            ...
            <div className="relative pl-6 sm:pl-8 border-l-2 border-[#C5A059]/30">
              ...
              <div className="bg-zinc-800/40 border border-zinc-700/30 rounded-xl p-5 sm:p-6">
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-[#C5A059] text-xl sm:text-2xl font-bold font-serif">2008</span>
                  <div>
                    <span className="text-white font-semibold text-sm block">Jogos Olímpicos de Pequim</span>
                    <span className="text-[10px] text-zinc-500">Transferência para Hong Kong</span>
                  </div>
                </div>
                <p className="text-[13px] text-zinc-400 leading-relaxed">
                  As provas equestres foram transferidas de Pequim para <strong className="text-white">Hong Kong</strong>,
                  devido a preocupações gerais com doenças equinas na China continental, incluindo a piroplasmose.
                  Hong Kong foi seleccionada pela sua infraestrutura de corridas, herdada da era colonial britânica,
                  que proporcionava instalações de quarentena e protocolos de biossegurança rigorosos.
                </p>
              </div>
            </div>
            ...
            <div className="relative pl-6 sm:pl-8 border-l-2 border-[#C5A059]/30">
              ...
              <div className="bg-zinc-800/40 border border-zinc-700/30 rounded-xl p-5 sm:p-6">
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-[#C5A059] text-xl sm:text-2xl font-bold font-serif">2010</span>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="bg-[#C5A059]/10 rounded-lg p-3 text-center">
                    ...
                    <p className="text-[10px] text-zinc-500 mt-1">cavalos piro-positivos competiram</p>
                  </div>
                  <div className="bg-zinc-700/30 rounded-lg p-3 text-center">
                    <span className="text-xl font-bold text-white">~500</span>
                    <p className="text-[10px] text-zinc-500 mt-1">cavalos em quarentena (Cincinnati)</p>
                  </div>
                </div>
                <p className="text-xs text-zinc-400">
                  Protocolos especiais aprovados pela USDA. Cavalos positivos tiveram estábulos e áreas de pastagem separadas.
                </p>
              </div>
            </div>
            ...
            <div className="relative pl-6 sm:pl-8 border-l-2 border-[#C5A059]/30">
              ...
              <div className="bg-zinc-800/40 border border-zinc-700/30 rounded-xl p-5 sm:p-6">
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-[#C5A059] text-xl sm:text-2xl font-bold font-serif">2018</span>
                  <div>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {["Isolamento separado", "Monitorização 24/7", "Inspecção diária carraças", "Tratamento acaricida", "Zero contacto com cavalos americanos"].map((p, i) => (
                    <span key={i} className="text-[10px] bg-green-500/10 text-green-300 border border-green-500/20 px-2.5 py-1 rounded-full">
                      <CheckCircle2 size={9} className="inline mr-1" />{p}
                    </span>
                  ))}
                </div>
                <p className="text-xs text-zinc-400">
                  Um estudo de 2017 determinou que o risco de propagação era <strong className="text-white">negligenciável a extremamente baixo</strong>.
                </p>
              </div>
            </div>
            ...
            <div className="relative pl-6 sm:pl-8 border-l-2 border-red-500/30">
              ...
              <div className="bg-red-500/5 border border-red-500/15 rounded-xl p-5 sm:p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <span className="text-white font-semibold text-sm block">Jogos Olímpicos de Tóquio</span>
                    <span className="text-[10px] text-red-400">Caso clínico documentado (estudo peer-reviewed)</span>
                  </div>
                </div>

                <div className="bg-zinc-900/60 rounded-lg p-4 mb-4">
                  <p className="text-xs text-zinc-500 mb-1 font-medium">Warmblood alemão, 15 anos</p>
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 text-xs text-zinc-400">
                    <div className="flex items-center gap-2">
                      <CheckCircle2 size={12} className="text-green-400" />
                      <span>Teste IFAT <strong className="text-green-400">negativo</strong> antes da partida</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock size={12} className="text-orange-400" />
                      <span><strong className="text-orange-400">3 dias</strong> após chegada</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <XCircle size={12} className="text-red-400" />
                      <span>Desenvolveu doença <strong className="text-red-400">aguda</strong></span>
                    </div>
                  </div>
                </div>
                ...
                <p className="text-xs text-zinc-400">
                  ...
                  Tratado com imidocarb. <strong className="text-green-400">Recuperou</strong> e regressou sem transmitir a doença.
                </p>
                <p className="text-[11px] text-zinc-500 mt-3 italic border-t border-zinc-700/30 pt-3">
                  Este caso demonstra como portadores assintomáticos podem testar negativo mas desenvolver doença aguda quando stressados pelo transporte.
                </p>
              </div>
            </div>
          </div>
    ...
    },
    {
      ...
      conteudo: (
        <div className="space-y-6 text-sm text-zinc-300 leading-relaxed">
          ...
          <p className="text-[15px] leading-relaxed">
            O <strong className="text-white">dipropionato de imidocarb</strong> é o tratamento de referência mundial.
            ...
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[9px] bg-[#C5A059]/20 text-[#C5A059] px-2 py-0.5 rounded-full font-bold">NOVO</span>
                </div>
                <p className="text-[13px] text-zinc-400 leading-relaxed">
                  ...
                  Pode ser administrado a cavalos de competição sob condições reguladas, com notificação
                  obrigatória aos delegados veterinários e restrições temporais relativas à competição.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4">
            ...
            <div className="bg-zinc-800/40 border border-zinc-700/30 rounded-xl overflow-hidden">
              <div className="bg-[#C5A059]/10 px-5 py-3 flex items-center gap-3 border-b border-zinc-700/30">
                ...
                <span className="text-sm font-semibold text-white">WOAH (Organização Mundial de Saúde Animal)</span>
              </div>
              <div className="p-5 space-y-3">
                {[
                  { icone: ShieldAlert, texto: "Doença de notificação obrigatória", destaque: true },
                  { icone: FileText, texto: "Regulada pelo Capítulo 12.7 do Código Sanitário para Animais Terrestres" },
                  // FONTE: WOAH Artigo 12.7.5 - "blood sample taken within the 14 days prior to shipment"
                  { icone: Clock, texto: "Testes negativos obrigatórios nos 14 dias antes do envio", destaque: true },
                  // FONTE: WOAH - 30 dias livres de carraças antes da colheita de sangue, e desde a colheita até ao envio
                  { icone: Bug, texto: "Animais livres de carraças 30 dias antes da colheita de sangue e desde a colheita até ao envio" },
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <item.icone size={14} className={`mt-0.5 flex-shrink-0 ${item.destaque ? 'text-[#C5A059]' : 'text-zinc-500'}`} />
                    <span className={`text-[13px] ${item.destaque ? 'text-white font-medium' : 'text-zinc-400'}`}>{item.texto}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-zinc-800/40 border border-zinc-700/30 rounded-xl overflow-hidden">
              ...
              <div className="p-5 space-y-3">
                {[
                  { icone: ShieldAlert, texto: "Classificada como doença animal estrangeira de notificação obrigatória", destaque: true },
                  { icone: FlaskConical, texto: "Todos os cavalos importados devem testar negativo (CFT e cELISA)" },
                  { icone: Clock, texto: "Se não-negativos: quarentena até 28 dias com reteste a cada 14 dias" },
                  { icone: Syringe, texto: "Programa de tratamento USDA-APHIS disponível para cavalos positivos", destaque: true },
                  { icone: CheckCircle2, texto: "Admissão temporária sob protocolos especiais para eventos internacionais" },
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <item.icone size={14} className={`mt-0.5 flex-shrink-0 ${item.destaque ? 'text-blue-400' : 'text-zinc-500'}`} />
                    <span className={`text-[13px] ${item.destaque ? 'text-white font-medium' : 'text-zinc-400'}`}>{item.texto}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-zinc-800/40 border border-zinc-700/30 rounded-xl overflow-hidden">
              ...
              <div className="p-5">
                <p className="text-[13px] text-zinc-400 leading-relaxed">
                  ...
                  de piroplasmose</strong>. No entanto, cavalos de países endémicos da UE
                  (como Portugal e Espanha) para países não-endémicos ou para nações fora da UE
                  devem cumprir os requisitos específicos do país importador.
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {["EUA", "Canadá", "Austrália", "Nova Zelândia", "Japão", "Reino Unido", "Irlanda", "Islândia"].map(pais => (
                <div key={pais} className="bg-green-500/5 border border-green-500/15 rounded-lg px-3 py-2.5 flex items-center gap-2">
        ...
        <div className="space-y-6 text-sm text-zinc-300 leading-relaxed">
            ...
            {[
              {
                ...
                texto: "Entre 18% e 33% dos Lusitanos em Portugal testa positivo para anticorpos, mesmo quando perfeitamente saudáveis, bloqueando a exportação."
              },
              {
                icone: Globe,
                corIcone: "text-orange-400",
                titulo: "Mercados fechados",
                texto: "As políticas dos EUA, Canadá, Austrália e outros mercados excluem uma porção significativa de Lusitanos criados em Portugal."
              },
              {
                icone: XCircle,
                corIcone: "text-red-500",
                titulo: "Sem solução simples",
                texto: "O tratamento é arriscado, não existe vacina, e os testes não distinguem infecção activa de imunidade natural por exposição em pastagem."
              },
              {
                icone: MapPin,
                corIcone: "text-yellow-400",
                titulo: "Problema estrutural",
                texto: "Enquanto a Península Ibérica permanecer endémica — inevitável pelo clima e carraças — esta barreira à exportação persistirá."
              },
              {
                icone: TrendingUp,
                corIcone: "text-[#C5A059]",
                titulo: "Duas categorias de mercado",
                texto: "Cavalos \"piro-free\" são comercializados como categoria premium, criando dois níveis de valor dentro da raça."
              },
            ].map((item, i) => (
              <div key={i} className={`bg-zinc-800/40 border border-zinc-700/30 rounded-xl p-5 ${i === 4 ? 'sm:col-span-2' : ''}`}>
                <div className="flex items-center gap-3 mb-3">
          ...
          <div className="bg-[#C5A059]/8 border border-[#C5A059]/20 rounded-xl p-5 sm:p-6">
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <div>
                <p className="text-[#C5A059] font-semibold text-sm mb-2">Para compradores internacionais</p>
                <p className="text-[13px] text-zinc-400 leading-relaxed">
                  Antes de adquirir um Lusitano em Portugal ou Espanha, solicite sempre o teste de piroplasmose actualizado.
                  Se planeia exportar para um país não-endémico, certifique-se de que o cavalo testa negativo antes de concluir a compra.
                  O custo do teste (<strong className="text-white">~60€ em Portugal</strong>) é insignificante comparado com o risco.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-400">
                Estudos científicos demonstram que cerca de <strong className="text-white">um em cada três cavalos</strong> em Portugal
                testa positivo para <em>Theileria equi</em>, mesmo sendo portadores assintomáticos saudáveis.
