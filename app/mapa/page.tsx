...
const LeafletMap = dynamic(() => import("@/components/LeafletMap"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full flex items-center justify-center bg-zinc-900/80 rounded-2xl">
      <Compass className="text-[#C5A059] animate-spin" size={32} />
    </div>
  ),
});
  ...
  return (
    <main className="min-h-screen bg-[#030303]">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 pb-16">
        ...
        {loading ? (
          ...
          <div className="grid lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-8">
              <div className="relative border border-white/10 rounded-2xl overflow-hidden" style={{ height: "600px" }}>
                <LeafletMap
              ...
              {selectedRegiao ? (
                <div className="sticky top-28 opacity-0 animate-[fadeSlideIn_0.5s_ease-out_forwards]">
                  <div className="mb-4 p-4 bg-zinc-900/80 border border-[#C5A059]/30 rounded-xl">
                    <div className="flex items-start justify-between">
                      ...
                      <button onClick={() => handleSelectRegiao(null)} className="p-1.5 text-zinc-500 hover:text-white bg-white/5 rounded-lg">
                        <X size={16} />
                      </button>
                    </div>
                  </div>
                ...
                <div className="sticky top-28 opacity-0 animate-[fadeSlideIn_0.5s_ease-out_forwards]">
                  <div className="mb-4 p-4 bg-zinc-900/80 border border-white/10 rounded-xl">
                    ...
                    <p className="text-zinc-500 text-xs">Selecione uma região ou clique num pin no mapa</p>
                  </div>
                  <div className="space-y-2">
                    {Object.entries(coudelariasPorRegiao).sort((a, b) => b[1].length - a[1].length).map(([regiao, list]) => (
                      <button key={regiao} onClick={() => handleSelectRegiao(regiao)} className="w-full p-3 bg-zinc-900/60 border border-white/5 rounded-lg hover:border-[#C5A059]/30 transition-colors text-left">
        ...
        {/* Modal de detalhe */}
        {selectedCoudelaria && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4 opacity-0 animate-[fadeSlideIn_0.5s_ease-out_forwards]" onClick={() => setSelectedCoudelaria(null)}>
            <div className="bg-zinc-900 border border-white/10 max-w-md w-full rounded-xl overflow-hidden opacity-0 animate-[fadeSlideIn_0.5s_ease-out_forwards]" style={{ animationDelay: "0.1s" }} onClick={e => e.stopPropagation()}>
