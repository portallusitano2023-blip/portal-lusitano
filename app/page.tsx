...
export default async function Page({ searchParams }) {
  // Garantimos que o modo dev funciona para o Francisco
  ...
  // O TEU SITE REAL (O que só tu vês para trabalhar)
  const products = await getProducts();
  
  return (
    <main className="min-h-screen bg-black text-white pt-40 px-10">
      <header className="mb-20 border-b border-zinc-900 pb-10">
        <p className="text-[#C5A059] text-[10px] uppercase font-bold mb-2 italic">Workspace Francisco</p>
        <h1 className="text-6xl font-serif italic">Produtos em Edição</h1>
      </header>

      <div className="grid grid-cols-2 gap-16">
        {products.map((p) => (
          <div key={p.id} className="border border-zinc-900 p-8 hover:border-[#C5A059]/30 transition-all">
            <h2 className="text-2xl font-serif mb-4">{p.title}</h2>
            <p className="text-[#C5A059] font-bold text-xl">
