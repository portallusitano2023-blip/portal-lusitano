...
import {
  ...
  GitBranch, RefreshCw, Sparkles, Target, BarChart3
} from "lucide-react";
...
const PELAGENS = [
  { value: "Ruço", label: "Ruço", desc: "Cinzento progressivo" },
  { value: "Castanho", label: "Castanho", desc: "Tradicional" },
  { value: "Preto", label: "Preto", desc: "Elegante" },
  { value: "Alazão", label: "Alazão", desc: "Avermelhado" },
  { value: "Baio", label: "Baio", desc: "Dourado" },
  { value: "Palomino", label: "Palomino", desc: "Creme dourado" },
  { value: "Isabelo", label: "Isabelo", desc: "Creme claro" },
  { value: "Cremelo", label: "Cremelo", desc: "Branco cremoso" }
];
...
export default function VerificadorCompatibilidadePage() {
  ...
  const calcular = () => {
    ...
    setTimeout(() => {
      ...
      const idadeScore = (idadeGaranhaoOk && idadeEguaOk) ? 15 :
                         (idadeGaranhaoOk || idadeEguaOk) ? 10 : 5;
      factores.push({
      ...
      if (egua.idade > 16) riscos.push({ texto: "Égua com idade avançada para reprodução", severidade: "medio" });
      if (egua.idade < 4) riscos.push({ texto: "Égua demasiado jovem", severidade: "alto" });
      if (garanhao.idade > 18) riscos.push({ texto: "Garanhão com idade avançada", severidade: "baixo" });
      total += idadeScore;
      ...
      if (difAltura > 10) riscos.push({ texto: `Diferença de altura significativa (${difAltura}cm)`, severidade: "baixo" });
      total += tamanhoScore;
      ...
      factores.push({
        ...
        score: tempScore,
        max: 10,
        tipo: tempScore >= 8 ? "excelente" : tempScore >= 6 ? "bom" : tempScore >= 4 ? "aviso" : "risco",
        descricao: "Combinação dos temperamentos dos progenitores"
      });
      if (tempScore <= 4) riscos.push({ texto: "Temperamentos potencialmente incompatíveis", severidade: "medio" });
      ...
      if (fertScore <= 2) riscos.push({ texto: "Fertilidade baixa pode dificultar concepção", severidade: "medio" });
      total += fertScore;
      ...
      const mesmaCoude = garanhao.coudelaria === egua.coudelaria && garanhao.coudelaria !== "Particular";
      const mesmaLinhagem = garanhao.linhagemFamosa === egua.linhagemFamosa && garanhao.linhagemFamosa !== "outra";
      ...
      if (coiPrevisto > 6.25) {
        riscos.push({ texto: `COI elevado previsto: ${coiPrevisto.toFixed(1)}%`, severidade: "alto" });
      ...
      if (garanhao.defeitos.includes("WFFS") && egua.defeitos.includes("WFFS")) {
        riscos.push({ texto: "RISCO CRÍTICO: 25% dos potros podem ter WFFS fatal", severidade: "alto" });
      }
      ...
      const nivel = total >= 85 ? "Excelente" :
                    total >= 70 ? "Muito Boa" :
                    total >= 55 ? "Boa" :
                    total >= 40 ? "Razoável" : "Problemática";

      // Recomendações
      const recomendacoes: string[] = [];
      if (coiPrevisto > 5) {
        recomendacoes.push("Considere um reprodutor de linhagem diferente para aumentar diversidade genética");
      }
      if (defeitosComuns.length > 0) {
      ...
      if (tempScore < 7) {
        recomendacoes.push("Potros podem herdar temperamento mais desafiante - preparar para trabalho de doma adequado");
      }
      if (total >= 80) {
        recomendacoes.push("Cruzamento promissor - considere registar potro no Livro de Nascimentos APSL");
      }
      if (andMedia >= 8 && confMedia >= 8) {
        recomendacoes.push("Potencial para potro de competição - considere plano de treino desde jovem");
      }
        ...
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              <span className="text-sm font-medium text-white block leading-tight">Verificador de Compatibilidade</span>
              <span className="text-xs text-zinc-500">Análise Genética para Reprodução</span>
            </div>
          </div>

          {resultado && (
            <button onClick={resetar} className="text-sm text-pink-400 hover:text-pink-300 transition-colors flex items-center gap-2">
              <RefreshCw size={14} />
              ...
              <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0">
                  <div
                    ...
                    style={{
                      backgroundImage: "url('https://images.unsplash.com/photo-1598974357801-cbca100e65d3?q=80&w=1920')",
                  ...
                  >
                    <div className="p-6 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                      ...
                      <p className="text-sm text-zinc-400">
                        Cálculo do Coeficiente de Consanguinidade previsto para ajudar
                        a evitar problemas genéticos na descendência.
                      </p>
                    </div>
                    ...
                    <div className="p-6 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                      ...
                      <p className="text-sm text-zinc-400">
                        Probabilidades de pelagem do potro baseadas nos genótipos
                        Extension, Agouti, Grey e Cream dos progenitores.
                      </p>
                    </div>
                    ...
                    <div className="p-6 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                      ...
                      <p className="text-sm text-zinc-400">
                        Identificação de defeitos genéticos comuns como WFFS, OCD
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-zinc-400 leading-relaxed">
                          ...
                          compatibilidade física, qualidade genética, conformação, andamentos, temperamento,
                          estado de saúde, fertilidade e defeitos genéticos conhecidos. O resultado inclui
                          previsões de BLUP, COI e pelagem do potro, bem como recomendações personalizadas.
                        </p>
                      </div>
                    </div>
                  </motion.div>
              ...
              <div className="bg-zinc-900/50 rounded-2xl p-6 border border-zinc-800 space-y-6">
                <div className="flex items-center gap-3 pb-4 border-b border-zinc-800">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-500">Informações para análise de compatibilidade</p>
                  </div>
                </div>
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs text-zinc-500 uppercase tracking-wider mb-2">Nome</label>
                    <input
                      type="text"
                      value={cavalo.nome}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-3 focus:border-pink-500 outline-none transition-colors"
                      />
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 text-sm">anos</span>
                    </div>
                  </div>
                  <div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-zinc-500 uppercase tracking-wider mb-2">Coudelaria / Origem</label>
                    <select
                      value={cavalo.coudelaria}
                      onChange={e => update("coudelaria", e.target.value)}
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-3 focus:border-pink-500 outline-none transition-colors"
                    >
                      {COUDELARIAS.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs text-zinc-500 uppercase tracking-wider mb-2">Qualidade da Linhagem</label>
                    <select
                ...
                {/* Linhagem Famosa */}
                <div>
                  <label className="block text-xs text-zinc-500 uppercase tracking-wider mb-3">Linhagem Principal</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {LINHAGENS_FAMOSAS.map(lin => (
                      ...
                      >
                        <span className={`block text-sm font-medium ${cavalo.linhagemFamosa === lin.value ? "text-pink-400" : "text-zinc-300"}`}>
                          {lin.label}
                        </span>
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  {[
                    ...
                    { field: "andamentos" as const, label: "Andamentos", desc: "Qualidade de movimento" },
                  ...
                  <div>
                    <label className="block text-xs text-zinc-500 uppercase tracking-wider mb-2">BLUP Index</label>
                    <input
                      type="number"
                      min="50"
                      max="150"
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs text-zinc-500 uppercase tracking-wider mb-2">Temperamento</label>
                    <select
                      value={cavalo.temperamento}
                      onChange={e => update("temperamento", e.target.value)}
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-3 focus:border-pink-500 outline-none"
                    >
                      {TEMPERAMENTOS.map(t => <option key={t.value} value={t.value}>{t.label} - {t.desc}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs text-zinc-500 uppercase tracking-wider mb-2">Fertilidade</label>
                    <select
                      value={cavalo.fertilidade}
                      onChange={e => update("fertilidade", e.target.value)}
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-3 focus:border-pink-500 outline-none"
                    >
                      {FERTILIDADES.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs text-zinc-500 uppercase tracking-wider mb-2">COI Conhecido (%)</label>
                    <input
                      type="number"
                      min="0"
                      max="25"
                    ...
                    {DEFEITOS_GENETICOS.map(d => (
                      <button
                        ...
                        className={`p-3 rounded-lg border text-left transition-all ${
                          cavalo.defeitos.includes(d.value)
                            ? d.risco === "alto" ? "border-red-500 bg-red-500/10" :
                              d.risco === "medio" ? "border-amber-500 bg-amber-500/10" :
                      ...
                      >
                        <span className={`block text-sm font-medium ${
                          cavalo.defeitos.includes(d.value)
                            ? d.risco === "alto" ? "text-red-400" :
                              d.risco === "medio" ? "text-amber-400" : "text-yellow-400"
                            : "text-zinc-300"
                        }`}>
                          {d.label}
                        </span>
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-zinc-900 via-zinc-900 to-zinc-800 p-8 border border-zinc-800 mb-6">
              ...
              <div className="relative z-10 text-center">
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full mb-4 ${
                  resultado.score >= 70 ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" :
                  resultado.score >= 50 ? "bg-amber-500/20 text-amber-400 border border-amber-500/30" :
                ...
                <div className="flex items-baseline justify-center gap-2 mb-2">
                  <span className="text-6xl sm:text-7xl font-light text-white">{resultado.score}</span>
            ...
            <div className="grid sm:grid-cols-3 gap-4 mb-6">
              <div className="bg-zinc-900/50 rounded-xl p-5 border border-zinc-800">
                ...
                <div className={`text-3xl font-light ${resultado.coi > 6.25 ? "text-amber-400" : "text-emerald-400"}`}>
                  {resultado.coi.toFixed(1)}%
                </div>
                <div className="text-xs text-zinc-500 mt-1">
                  {resultado.coi <= 3 ? "Excelente" : resultado.coi <= 6.25 ? "Aceitável" : "Elevado"}
                </div>
              </div>
              ...
              <div className="bg-zinc-900/50 rounded-xl p-5 border border-zinc-800">
                ...
                <div className="text-3xl font-light text-pink-400">
                  {resultado.altura.min}-{resultado.altura.max}<span className="text-lg">cm</span>
                </div>
                ...
                {resultado.riscos.map((r, i) => (
                  <div
                    ...
                    className={`p-4 rounded-lg flex items-center gap-3 ${
                      r.severidade === "alto" ? "bg-red-500/10 border border-red-500/30" :
                      r.severidade === "medio" ? "bg-amber-500/10 border border-amber-500/30" :
                      "bg-yellow-500/10 border border-yellow-500/30"
                    }`}
                  >
                    <AlertTriangle size={18} className={
                      r.severidade === "alto" ? "text-red-400" :
                      r.severidade === "medio" ? "text-amber-400" : "text-yellow-400"
                    } />
                  ...
                  <div key={i}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-sm font-bold ${
                        f.tipo === "excelente" ? "text-emerald-400" :
                        f.tipo === "bom" ? "text-blue-400" :
                        f.tipo === "aviso" ? "text-amber-400" :
                        f.tipo === "risco" ? "text-red-400" : "text-zinc-400"
                      }`}>
                        {f.score}/{f.max}
                      </span>
                    </div>
            ...
            <div className="p-4 bg-zinc-900/30 rounded-xl border border-zinc-800/50">
              <p className="text-xs text-zinc-500 leading-relaxed">
                <strong className="text-zinc-400">Aviso:</strong> Esta análise é uma ferramenta de apoio à decisão
                e não substitui a consulta de um veterinário especializado em reprodução equina ou geneticista.
                Os resultados são estimativas baseadas nos dados fornecidos. Recomendamos sempre realizar
