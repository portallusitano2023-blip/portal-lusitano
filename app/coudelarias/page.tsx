...
export default function CoudelariasPage() {
  ...
  const coudelarias = [
    ...
    },
    {
      ...
      image:
        "https://images.unsplash.com/photo-1629814403061-0083a54d6d60?q=80&w=1000&auto=format&fit=crop",
      description:
        t.nav.home === "Home"
    ...
    <>
      ...
      <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)] pt-32 pb-20 px-6 selection:bg-[var(--gold)] selection:text-black">
        ...
        <div className="max-w-4xl mx-auto text-center mb-20">
          <span className="text-[var(--gold)] text-[9px] uppercase tracking-[0.4em] font-bold block mb-6 animate-fade-in">
            {t.studs.directory}
          </span>
          <h1 className="text-5xl md:text-7xl font-serif text-[var(--foreground)] tracking-tight mb-6">
            {t.studs.title.split(" ").slice(0, -1).join(" ")}{" "}
            <span className="italic font-light text-[var(--foreground-muted)]">
              {t.studs.title.split(" ").slice(-1)}
            </span>
          </h1>
          <p className="text-[var(--foreground-secondary)] font-light max-w-2xl mx-auto text-sm leading-loose">
            {t.studs.subtitle}
          </p>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          {coudelarias.map((item) => (
            <div
              key={item.id}
              className="group relative bg-[var(--background-secondary)]/20 border border-[var(--border)] overflow-hidden hover:border-[var(--gold)]/50 transition-all duration-700"
            >
              {/* Imagem */}
              <div className="h-64 w-full overflow-hidden relative">
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <p className="text-[var(--gold)] text-[9px] uppercase tracking-widest mb-2">
                    {item.location}
                  </p>
                  <h2 className="text-3xl font-serif text-[var(--foreground)] italic">
                    {item.name}
                  </h2>
                </div>
              </div>
              ...
              {/* Descricao e Botao */}
              <div className="p-8">
                <p className="text-[var(--foreground-muted)] text-sm font-light leading-relaxed mb-6">
                  {item.description}
                </p>
                <div className="flex justify-end">
                  <button className="text-[9px] uppercase tracking-[0.2em] text-[var(--foreground)] group-hover:text-[var(--gold)] transition-colors flex items-center gap-2">
        ...
        {/* Rodape da seccao */}
        <div className="text-center mt-20 border-t border-[var(--border)] pt-10">
