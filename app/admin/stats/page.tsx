  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white pt-32 pb-20 font-sans">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex justify-between items-end mb-12 border-b border-zinc-800 pb-8 font-sans">
          <div>
            <span className="text-[#C5A059] uppercase tracking-[0.4em] text-[10px] font-bold block mb-2">Consola Administrativa</span>
            <h1 className="text-4xl font-serif italic">Performance do Portal</h1>
          </div>
          <div className="text-right">
            <p className="text-green-600 text-sm font-mono animate-pulse">● Live Data</p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <div className="bg-zinc-900/30 border border-zinc-800 p-8">
            <p className="text-zinc-500 uppercase tracking-widest text-[10px] mb-2 font-sans">Leads</p>
            <p className="text-4xl font-serif text-[#C5A059]">{data.totalSubscritores}</p>
          </div>
          <div className="bg-zinc-900/30 border border-zinc-800 p-8">
            <p className="text-zinc-500 uppercase tracking-widest text-[10px] mb-2 font-sans">Propostas</p>
          ...
          <div className="bg-zinc-900/30 border border-[#C5A059]/30 p-8">
            <p className="text-zinc-500 uppercase tracking-widest text-[10px] mb-2 font-sans">Volume Total</p>
            <p className="text-4xl font-serif text-[#C5A059]">{(data.valorTotal || 0).toLocaleString('pt-PT')} €</p>
          </div>
          <div className="bg-zinc-900/30 border border-zinc-800 p-8">
            <p className="text-zinc-500 uppercase tracking-widest text-[10px] mb-2 font-sans">Cavalos</p>
            <p className="text-4xl font-serif text-white">{data.totalCavalos}</p>
          </div>
        </div>

        <div className="bg-zinc-900/10 border border-zinc-900 p-8">
          <h3 className="font-serif text-xl mb-6">Últimas Atividades Reais</h3>
          <div className="space-y-4">
            {data.ultimasLicitacoes?.map((lic, i) => (
              <div key={i} className="flex justify-between items-center p-4 border-b border-zinc-900 font-sans">
                <div className="text-sm">
                  <p className="font-bold text-zinc-200">{lic.nome}</p>
