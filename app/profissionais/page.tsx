...
import {
  ...
  Truck, Scale, BookOpen, Mic, Video, Instagram, Facebook,
  Linkedin, Youtube, AlertTriangle, Trophy, Sparkles, FileText,
  PlayCircle, CalendarDays, GraduationCap as Graduation, Building,
  Siren, Activity, PieChart, BarChart3, MapPinned, Languages,
  Verified, BadgeCheck, Dumbbell, Thermometer, Microscope, Pill,
  Scan, Wrench, PaintBucket, Lightbulb, HandHeart, UserCheck
} from "lucide-react";
...
type CategoriaProf = "veterinario" | "ferrador" | "treinador" | "fotografo" | "tosquiador" |
  "dentista" | "quiropratico" | "nutricionista" | "transportador" | "juiz" |
  "inseminador" | "instrutor" | "seleiro" | "massagista";
...
const categorias: { id: CategoriaProf | "todos"; label: string; icon: typeof Briefcase; descricao: string }[] = [
  { id: "todos", label: "Todos", icon: Briefcase, descricao: "Ver todos os profissionais" },
  { id: "veterinario", label: "Veterinários", icon: Stethoscope, descricao: "Medicina equina geral e especializada" },
  { id: "ferrador", label: "Ferradores", icon: Hammer, descricao: "Ferração normal, ortopédica e correctiva" },
  { id: "treinador", label: "Treinadores", icon: GraduationCap, descricao: "Dressage, Working Equitation, Salto" },
  { id: "dentista", label: "Dentistas Equinos", icon: Scan, descricao: "Odontologia equina especializada" },
  { id: "quiropratico", label: "Quiropráticos", icon: Activity, descricao: "Quiroprática e osteopatia equina" },
  { id: "nutricionista", label: "Nutricionistas", icon: Pill, descricao: "Nutrição e suplementação equina" },
  { id: "inseminador", label: "Inseminadores", icon: Microscope, descricao: "Reprodução assistida e IA" },
  { id: "fotografo", label: "Fotógrafos", icon: Camera, descricao: "Fotografia e vídeo equestre" },
  { id: "transportador", label: "Transportadores", icon: Truck, descricao: "Transporte nacional e internacional" },
  ...
  { id: "massagista", label: "Massagistas", icon: HandHeart, descricao: "Massagem e fisioterapia equina" },
];
...
const VERIFICACAO_CONFIG = {
  basico: { cor: "text-zinc-400", bg: "bg-zinc-800", icon: Shield, label: "Perfil Básico", descricao: "Perfil registado" },
  verificado: { cor: "text-blue-400", bg: "bg-blue-500/20", icon: ShieldCheck, label: "Verificado", descricao: "Identidade confirmada" },
  certificado: { cor: "text-[#C5A059]", bg: "bg-[#C5A059]/20", icon: Award, label: "Certificado", descricao: "Credenciais validadas" },
  expert: { cor: "text-purple-400", bg: "bg-purple-500/20", icon: Crown, label: "Expert", descricao: "Top profissional" }
};
...
const profissionaisDB: Profissional[] = [
  {
    ...
    descricao: "Especialista em medicina equina com foco em ortopedia e medicina desportiva. Mais de 20 anos de experiência com cavalos Lusitanos de alta competição. Veterinário oficial em múltiplos campeonatos nacionais e internacionais.",
    avaliacao: 4.9,
    numAvaliacoes: 234,
    servicos: [
      "Consultas gerais", "Cirurgia ortopédica", "Medicina desportiva",
      "Exames pré-compra completos", "Emergências 24h", "Radiologia digital",
      "Ecografia musculoesquelética", "Artroscopia", "PRP/Células estaminais",
      "Shockwave therapy", "Mesoterapia"
    ],
    nivelVerificacao: "expert",
    experienciaAnos: 22,
    especializacoes: [
      { nome: "Ortopedia Equina", nivel: "especialista", certificado: "ECVS Diplomate", anoObtencao: 2008, instituicao: "European College of Veterinary Surgeons" },
      { nome: "Medicina Desportiva", nivel: "especialista", certificado: "FEI Official Veterinarian", anoObtencao: 2010, instituicao: "FEI" },
      { nome: "Imagiologia Avançada", nivel: "avancado", certificado: "Diplomate ACVR", anoObtencao: 2012, instituicao: "American College of Veterinary Radiology" },
      { nome: "Cirurgia Artroscópica", nivel: "especialista", certificado: "Advanced Arthroscopy", anoObtencao: 2015, instituicao: "Colorado State University" },
    ...
    testemunhos: [
      { cliente: "Coudelaria Real de Alter", texto: "O Dr. António salvou a carreira desportiva do nosso garanhão principal com uma cirurgia artroscópica impecável. Profissionalismo de nível mundial.", data: "2024-01", avaliacao: 5, verificado: true, cavalo: "Altivo II" },
      { cliente: "João Moura Jr.", texto: "Acompanha os meus cavalos de toureio há 15 anos. Sempre disponível, mesmo às 3 da manhã. É o melhor.", data: "2024-02", avaliacao: 5, verificado: true },
      { cliente: "Coudelaria Veiga", texto: "Exames pré-compra extremamente detalhados. Encontrou problemas que outros veterinários não viram.", data: "2023-11", avaliacao: 5, verificado: true },
      { cliente: "Ana R.", texto: "Tratou a tendinite crónica do meu cavalo com PRP. Resultados fantásticos!", data: "2024-03", avaliacao: 5, verificado: false }
    ],
    casosSucesso: [
      { titulo: "Recuperação de fratura de boleto em campeão", descricao: "Cavalo Grand Prix com fratura por stress complexa do terceiro metacarpo", resultado: "Regresso à competição internacional após 14 meses - venceu CDI3* Lisboa", data: "2023", destaque: true, imagens: 8 },
      { titulo: "Tratamento inovador de OCD bilateral", descricao: "Poldro de 2 anos com OCD em ambos os curvilhões, grau III", resultado: "Aprovação APSL com pontuação 75/80, exportado para Alemanha", data: "2024", imagens: 5 },
      { titulo: "Cirurgia de cólica complicada", descricao: "Torção intestinal de 360° em égua prenha", resultado: "Égua e poldro salvos, poldro nasceu saudável 4 meses depois", data: "2023", destaque: true, imagens: 3 }
    ],
    metricas: {
    ...
    formacao: [
      { titulo: "Licenciatura em Medicina Veterinária", instituicao: "Faculdade de Medicina Veterinária - ULisboa", ano: 2002, tipo: "licenciatura" },
      { titulo: "Mestrado em Cirurgia Equina", instituicao: "Royal Veterinary College - Londres", ano: 2005, tipo: "mestrado" },
      { titulo: "Residência ECVS", instituicao: "University of Liverpool", ano: 2008, tipo: "certificacao" },
      { titulo: "Advanced Equine Arthroscopy", instituicao: "Colorado State University", ano: 2015, tipo: "curso" }
    ],
    publicacoes: [
      { titulo: "Avanços na cirurgia artroscópica em cavalos Lusitanos", revista: "Equine Veterinary Journal", ano: 2022, tipo: "artigo" },
      { titulo: "Manual de Ortopedia Equina Prática", ano: 2020, tipo: "livro" },
      { titulo: "Série: Exame Ortopédico Completo", ano: 2023, tipo: "video", link: "youtube.com/@drsilvavet" }
    ],
    premios: [
      { titulo: "Veterinário do Ano - APSL", ano: 2023, entidade: "Associação Portuguesa de criadores do cavalo Puro Sangue Lusitano" },
      { titulo: "Prémio Excelência em Cirurgia Equina", ano: 2021, entidade: "Sociedade Portuguesa de Medicina Veterinária" },
    ...
    equipamento: [
      { nome: "Raio-X Digital Portátil", descricao: "Sound Eklin - alta resolução para diagnóstico em campo" },
      { nome: "Ecógrafo Esaote MyLab", descricao: "Ecografia musculoesquelética de alta frequência" },
      { nome: "Endoscópio Karl Storz", descricao: "Para diagnóstico de vias respiratórias" },
      { nome: "Equipamento Shockwave", descricao: "Swiss DolorClast para terapia por ondas de choque" }
    ],
    cursosOferecidos: [
      { titulo: "Exame Ortopédico para Criadores", duracao: "2 dias", preco: "€350", proximaData: "2024-05-15", vagas: 12 },
      { titulo: "Primeiros Socorros Equinos", duracao: "1 dia", preco: "€150", proximaData: "2024-04-20", vagas: 20 }
    ],
  ...
  },
  {
    ...
    descricao: "Mestre ferrador com certificação internacional EFFA Level 4 e AFA Certified Journeyman Farrier. Especializado em correção de aprumos, ferradura ortopédica e casos complexos. Formador certificado com alunos em Portugal, Espanha e Brasil.",
    avaliacao: 4.9,
    numAvaliacoes: 189,
    servicos: [
      ...
      "Ferração para cavalos de desporto", "Tratamento de laminite", "Correção de defeitos de casco",
      "Ferração para navicular", "Consultoria a distância"
    ],
    nivelVerificacao: "expert",
    experienciaAnos: 25,
    especializacoes: [
      { nome: "Ferração Ortopédica", nivel: "especialista", certificado: "EFFA Level 4", anoObtencao: 2010, instituicao: "European Federation of Farriers Associations" },
      { nome: "Correção de Aprumos", nivel: "especialista", certificado: "AFA Certified Journeyman", anoObtencao: 2012, instituicao: "American Farrier Association" },
      { nome: "Tratamento de Laminite", nivel: "especialista", certificado: "Certified in Equine Podiatry", anoObtencao: 2015, instituicao: "IAEP" },
    ...
    testemunhos: [
      { cliente: "Coudelaria Vale do Tejo", texto: "Trabalho impecável. Transformou completamente os cascos de um cavalo que dávamos como perdido por laminite crónica.", data: "2024-01", avaliacao: 5, verificado: true },
      { cliente: "Centro Hípico Nacional", texto: "O Manuel ferra todos os nossos cavalos de competição. Nunca tivemos um problema de casco desde que trabalha connosco.", data: "2024-02", avaliacao: 5, verificado: true },
      { cliente: "Dr. António Silva", texto: "Recomendo o Manuel para todos os casos ortopédicos complexos. Colaboramos há 15 anos com excelentes resultados.", data: "2023-12", avaliacao: 5, verificado: true }
    ],
    casosSucesso: [
      { titulo: "Recuperação de laminite grau IV", descricao: "Cavalo com rotação de P3 de 18 graus, prognóstico reservado", resultado: "Após 18 meses de trabalho intensivo, cavalo voltou ao trabalho leve - passeios de 1 hora", data: "2023", destaque: true, imagens: 12 },
      { titulo: "Correção de aprumos em poldro", descricao: "Poldro com deformação angular severa (valgus) aos 6 meses", resultado: "Aprumos corrigidos aos 18 meses sem necessidade de cirurgia", data: "2024", imagens: 8 }
    ],
    metricas: {
    ...
    formacao: [
      { titulo: "Curso de Ferrador", instituicao: "Escola de Artes e Ofícios", ano: 1999, tipo: "curso" },
      { titulo: "EFFA Level 4 Certification", instituicao: "EFFA", ano: 2010, tipo: "certificacao" },
      { titulo: "Equine Podiatry Certification", instituicao: "IAEP", ano: 2015, tipo: "certificacao" },
      { titulo: "Advanced Laminitis Treatment", instituicao: "Rood & Riddle Equine Hospital", ano: 2018, tipo: "curso" }
    ],
    equipamento: [
      { nome: "Forja Portátil NC Tool", descricao: "Forja a gás de alta temperatura para trabalho em campo" },
      { nome: "Raio-X Portátil", descricao: "Para avaliação de casos ortopédicos" },
      { nome: "Kit de Palmilhas Impressão 3D", descricao: "Sistema de scan e impressão para palmilhas personalizadas" }
    ],
    cursosOferecidos: [
      { titulo: "Introdução à Ferração Ortopédica", duracao: "3 dias", preco: "€600", proximaData: "2024-06-01", vagas: 8 },
      { titulo: "Tratamento de Laminite para Veterinários e Ferradores", duracao: "2 dias", preco: "€450", proximaData: "2024-07-15", vagas: 10 }
    ],
    redesSociais: {
  ...
  },
  {
    ...
    descricao: "Cavaleira internacional de Dressage com múltiplas participações em campeonatos europeus e mundiais. Formação de cavalos jovens desde o desbaste até Grand Prix. Ex-membro da Seleção Nacional. Especialista em biomecânica e correção de cavaleiros.",
    avaliacao: 5.0,
    numAvaliacoes: 156,
    servicos: [
      "Treino de cavalos jovens", "Preparação Grand Prix", "Aulas particulares",
      "Clínicas e workshops", "Desbaste", "Consultoria técnica", "Preparação para concursos",
      "Correção de cavaleiros", "Treino de freestyle", "Análise de vídeo",
      "Programa de competição"
    ],
    nivelVerificacao: "expert",
    experienciaAnos: 28,
    especializacoes: [
      { nome: "Dressage - Grand Prix", nivel: "especialista", certificado: "FEI Level 3 Coach", anoObtencao: 2008, instituicao: "FEI" },
      { nome: "Formação de Jovens Cavalos", nivel: "especialista", certificado: "German FN Trainer A", anoObtencao: 2005, instituicao: "Deutsche Reiterliche Vereinigung" },
      { nome: "Biomecânica Equina", nivel: "especialista", certificado: "Certified Equine Biomechanics", anoObtencao: 2012 },
      { nome: "Equitação Clássica", nivel: "especialista", certificado: "Spanish Riding School", anoObtencao: 2010, instituicao: "Escola Espanhola de Viena" }
    ],
    credenciais: [
    ...
    testemunhos: [
      { cliente: "Sofia M.", texto: "A Maria transformou-me e ao meu cavalo. Fomos de São Jorge a Grand Prix em 4 anos com uma metodologia incrível.", data: "2024-02", avaliacao: 5, verificado: true },
      { cliente: "Centro Equestre da Marinha", texto: "Os nossos cavalos jovens treinados pela Maria vendem-se em dias. Metodologia de desbaste exemplar.", data: "2024-01", avaliacao: 5, verificado: true },
      { cliente: "Pedro L.", texto: "Metodologia clara e resultados visíveis. Conseguiu corrigir problemas de postura que tinha há 10 anos.", data: "2023-12", avaliacao: 5, verificado: true }
    ],
    casosSucesso: [
      { titulo: "Formação completa Grand Prix", descricao: "Lusitano iniciou desbaste aos 4 anos, treino exclusivo", resultado: "Campeão Nacional Grand Prix aos 10 anos, 8º no CDI-W Lisboa", data: "2023", destaque: true },
      { titulo: "Recuperação de cavalo problemático", descricao: "Cavalo de 8 anos com problemas graves de confiança após acidentes", resultado: "Hoje compete a nível Small Tour com proprietária amadora", data: "2024" }
    ],
    metricas: {
    ...
    formacao: [
      { titulo: "German FN Bereiter", instituicao: "Westfälische Reit- und Fahrschule", ano: 2000, tipo: "certificacao" },
      { titulo: "German FN Trainer A", instituicao: "Deutsche Reiterliche Vereinigung", ano: 2005, tipo: "certificacao" },
      { titulo: "Stage na Escola Espanhola de Viena", instituicao: "Spanish Riding School", ano: 2010, tipo: "curso" },
    ...
    publicacoes: [
      ...
      { titulo: "Série: Do Desbaste ao Grand Prix", ano: 2023, tipo: "video", link: "youtube.com/@mariasantosdressage" }
    ],
    premios: [
    ...
    cursosOferecidos: [
      { titulo: "Clínica de Dressage - Todos os níveis", duracao: "2 dias", preco: "€300", proximaData: "2024-05-20", vagas: 10 },
      { titulo: "Curso Intensivo - Preparação para concurso", duracao: "5 dias", preco: "€800", proximaData: "2024-06-10", vagas: 6 }
    ],
    seguroProfissional: true,
    destaque: true,
    disponivel: true,
  ...
  },
  {
    ...
    descricao: "Especialista em reprodução equina com formação internacional nos EUA e Europa. Pioneira em Portugal em transferência de embriões e ICSI. Acompanhamento completo desde a inseminação até ao nascimento do poldro.",
    avaliacao: 4.9,
    numAvaliacoes: 178,
    servicos: [
      "Inseminação artificial fresco/refrigerado/congelado", "Transferência de embriões",
      "ICSI (injeção intracitoplasmática)", "Ecografia reprodutiva", "Acompanhamento de gestação",
      "Neonatologia", "Congelação de sémen", "Avaliação de sémen",
      "Tratamento de infertilidade", "Sincronização de cios", "Parto assistido"
    ],
    nivelVerificacao: "expert",
    experienciaAnos: 15,
    especializacoes: [
      { nome: "Reprodução Assistida", nivel: "especialista", certificado: "ACT Diplomate", anoObtencao: 2014, instituicao: "American College of Theriogenologists" },
      { nome: "Transferência de Embriões", nivel: "especialista", certificado: "IETS Certified", anoObtencao: 2016, instituicao: "International Embryo Technology Society" },
      { nome: "Neonatologia Equina", nivel: "especialista", certificado: "Neonatal Intensive Care", anoObtencao: 2018, instituicao: "University of Pennsylvania" },
      { nome: "ICSI Equino", nivel: "especialista", certificado: "Advanced ART", anoObtencao: 2020, instituicao: "Texas A&M University" }
    ],
    credenciais: [
    ...
    testemunhos: [
      { cliente: "Coudelaria Veiga", texto: "A Dra. Carla conseguiu 5 gestações de uma égua velha que nunca tinha conseguido emprenhar. Trabalho extraordinário.", data: "2024-01", avaliacao: 5, verificado: true },
      { cliente: "Stud Interagro Brasil", texto: "Trouxemos a Dra. Carla ao Brasil para um programa de ICSI. Taxa de sucesso acima de qualquer expectativa.", data: "2023-10", avaliacao: 5, verificado: true }
    ],
    casosSucesso: [
      { titulo: "Programa ICSI com sucesso recorde", descricao: "12 éguas doadoras, garanhão falecido (sémen congelado)", resultado: "9 gestações confirmadas, 8 poldros nascidos saudáveis", data: "2024", destaque: true },
      { titulo: "Salvamento de poldro prematuro", descricao: "Poldro nascido aos 300 dias, 28kg", resultado: "Após 45 dias de cuidados intensivos, poldro saudável hoje com 2 anos", data: "2022", destaque: true }
    ],
    metricas: {
    ...
    equipamento: [
      ...
      { nome: "Laboratório ICSI Completo", descricao: "Equipamento Eppendorf para micromanipulação" },
  ...
  },
  {
    ...
    descricao: "Dentista equino certificado pela BEVA e IAED com foco em odontologia preventiva e tratamento de patologias complexas. Equipamento de última geração incluindo radiografia digital oral.",
    avaliacao: 4.8,
    numAvaliacoes: 145,
    servicos: [
      "Exame oral completo", "Nivelamento dentário", "Extração de dentes de lobo",
      "Tratamento de diastemas", "Radiografia oral digital", "Tratamento de cáries",
      "Remoção de pontas de esmalte", "Tratamento de EOTRH", "Odontologia geriátrica"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 12,
    especializacoes: [
      { nome: "Odontologia Equina", nivel: "especialista", certificado: "BEVA Dental Technician", anoObtencao: 2014, instituicao: "British Equine Veterinary Association" },
      { nome: "Radiografia Oral", nivel: "avancado", certificado: "IAED Certified", anoObtencao: 2016, instituicao: "International Association of Equine Dentistry" },
  ...
  },
  {
    ...
    descricao: "Cavaleiro profissional de Equitação de Trabalho com múltiplos títulos nacionais e internacionais. Especialista em desbaste tradicional à portuguesa e preparação de cavalos para toureio a cavalo.",
    avaliacao: 4.9,
    numAvaliacoes: 112,
    servicos: [
      "Treino específico Working Equitation", "Preparação para competição",
      "Clínicas e workshops", "Desbaste tradicional", "Treino de gado",
      "Preparação para toureio", "Treino de conjuntos amadores"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 22,
    especializacoes: [
      { nome: "Equitação de Trabalho", nivel: "especialista", certificado: "WAWE International Judge", anoObtencao: 2015, instituicao: "WAWE" },
    ...
    testemunhos: [
      { cliente: "Francisco T.", texto: "O Pedro preparou o meu cavalo para o campeonato nacional. Resultado: medalha de bronze na primeira participação!", data: "2024-01", avaliacao: 5, verificado: true }
    ],
    metricas: {
  ...
  },
  {
    ...
    descricao: "Fotógrafo equestre premiado com publicações em revistas internacionais. Especializado em retratos artísticos de cavalos Lusitanos, cobertura de eventos e produção de vídeo promocional de alta qualidade.",
    avaliacao: 4.9,
    numAvaliacoes: 89,
    servicos: [
      "Sessões em coudelaria", "Cobertura de competições", "Retratos artísticos",
      "Vídeo promocional 4K", "Catálogos de venda", "Fotos para stud books",
      "Fotos aéreas com drone", "Edição profissional", "Impressão fine art"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 15,
    especializacoes: [
      ...
      { nome: "Fotografia Aérea", nivel: "avancado", certificado: "Piloto Drone A2", anoObtencao: 2021 },
  ...
  },
  {
    ...
    descricao: "Quiroprática equina certificada com formação na IVCA. Especializada em diagnóstico e tratamento de disfunções biomecânicas, com foco em cavalos de desporto e recuperação de lesões.",
    avaliacao: 4.8,
    numAvaliacoes: 98,
    servicos: [
      "Avaliação quiropráctica completa", "Ajustes quiropráticos", "Osteopatia equina",
      "Mobilização articular", "Tratamento de dor lombar", "Avaliação de performance",
      "Reabilitação pós-lesão", "Manutenção preventiva"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 10,
    especializacoes: [
      { nome: "Quiroprática Equina", nivel: "especialista", certificado: "IVCA Certified", anoObtencao: 2016, instituicao: "International Veterinary Chiropractic Association" },
      { nome: "Osteopatia Animal", nivel: "avancado", certificado: "AOA Certified", anoObtencao: 2018 },
    ...
    testemunhos: [
      { cliente: "Maria Santos (Treinadora)", texto: "A Sofia é essencial na minha equipa. Os cavalos movem-se muito melhor após os tratamentos. Recomendo a 100%.", data: "2024-02", avaliacao: 5, verificado: true }
    ],
    metricas: {
  ...
  },
  {
    ...
    descricao: "Especialista em nutrição equina com doutoramento em fisiologia digestiva equina. Desenvolve planos nutricionais personalizados para cavalos de desporto, reprodução e recuperação.",
    avaliacao: 4.7,
    numAvaliacoes: 67,
    servicos: [
      "Avaliação nutricional completa", "Planos alimentares personalizados",
      "Suplementação desportiva", "Nutrição reprodutiva", "Controlo de peso",
      "Tratamento de úlceras gástricas", "Análise de feno e rações",
      "Consultoria a coudelarias"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 8,
    especializacoes: [
      { nome: "Nutrição Desportiva", nivel: "especialista", certificado: "Equine Nutritionist", anoObtencao: 2018, instituicao: "University of Edinburgh" },
      { nome: "Fisiologia Digestiva", nivel: "especialista", certificado: "PhD", anoObtencao: 2020, instituicao: "FMV-ULisboa" },
    ...
    publicacoes: [
      ...
      { titulo: "Impact of Diet on Gastric Ulcers in Performance Horses", revista: "Equine Veterinary Journal", ano: 2021, tipo: "artigo" }
    ],
    seguroProfissional: true,
    disponivel: true,
  ...
  },
  {
    ...
    descricao: "Empresa de transporte equino com 20 anos de experiência. Frota moderna com veículos climatizados e sistemas de monitorização GPS. Transporte para toda a Europa com documentação incluída.",
    avaliacao: 4.8,
    numAvaliacoes: 234,
    servicos: [
      "Transporte nacional", "Transporte internacional", "Transporte urgente",
      "Transporte de competição", "Transporte de garanhões", "Quarentena incluída",
      "Documentação completa", "Seguro de transporte", "Paragens programadas"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 20,
    especializacoes: [
      { nome: "Transporte Internacional", nivel: "especialista", certificado: "TRACES Certified", anoObtencao: 2010 },
  ...
  },
  {
    ...
    descricao: "Juiz internacional FEI 4* de Dressage e juiz oficial da APSL para concursos morfológicos. Vasta experiência em campeonatos europeus e mundiais. Disponível para clínicas de formação de juízes.",
    avaliacao: 5.0,
    numAvaliacoes: 45,
    servicos: [
      "Juiz de dressage FEI", "Juiz de morfologia APSL", "Clínicas de formação",
      "Avaliação técnica de cavalos", "Consultoria de criação", "Workshops para cavaleiros"
    ],
    nivelVerificacao: "expert",
    experienciaAnos: 30,
    especializacoes: [
      { nome: "Juiz de Dressage", nivel: "especialista", certificado: "FEI 4*", anoObtencao: 2010, instituicao: "FEI" },
      { nome: "Juiz de Morfologia", nivel: "especialista", certificado: "APSL Official", anoObtencao: 2005, instituicao: "APSL" }
    ],
    credenciais: [
  ...
  },
  {
    ...
    descricao: "Técnico de inseminação artificial certificado com mais de 15 anos de experiência. Trabalho em colaboração com os principais veterinários reprodutores. Especializado em sémen congelado.",
    avaliacao: 4.7,
    numAvaliacoes: 123,
    servicos: [
      "Inseminação artificial fresco", "Inseminação refrigerado", "Inseminação congelado",
      "Colheita de sémen", "Avaliação de sémen", "Sincronização de cios",
      "Detecção de ovulação"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 15,
    especializacoes: [
      { nome: "Inseminação Artificial", nivel: "especialista", certificado: "AETE Certified", anoObtencao: 2012, instituicao: "Association of Embryo Technology in Europe" },
  ...
  },
  {
    ...
    descricao: "Instrutora certificada pela FEP com especialização em equitação terapêutica e ensino infantil. Metodologia gentil baseada em comunicação e confiança. Centro hípico próprio com 15 cavalos de escola.",
    avaliacao: 4.9,
    numAvaliacoes: 178,
    servicos: [
      "Aulas de iniciação", "Aulas avançadas", "Equitação terapêutica",
      "Aulas para crianças", "Preparação para exames FEP", "Passeios a cavalo",
      "Festas de aniversário", "Campos de férias"
    ],
    nivelVerificacao: "verificado",
    experienciaAnos: 12,
    especializacoes: [
      { nome: "Ensino de Equitação", nivel: "avancado", certificado: "FEP Nível II", anoObtencao: 2015, instituicao: "Federação Equestre Portuguesa" },
      { nome: "Equitação Terapêutica", nivel: "especialista", certificado: "ANDE-Brasil", anoObtencao: 2018 },
    ...
    testemunhos: [
      { cliente: "Mãe da Beatriz (8 anos)", texto: "A Catarina é incrível com crianças. A minha filha adora as aulas e evolui imenso!", data: "2024-02", avaliacao: 5, verificado: true }
    ],
    metricas: {
      tempoResposta: "menos de 6 horas",
      taxaSatisfacao: 98,
  ...
  },
  {
    ...
    descricao: "Seleiro artesanal de 3ª geração, especializado em selaria portuguesa tradicional. Trabalho 100% artesanal com couro de primeira qualidade. Fornecedor de coudelarias reais e cavaleiros profissionais.",
    avaliacao: 5.0,
    numAvaliacoes: 67,
    servicos: [
      "Selas portuguesas por medida", "Arreios completos", "Reparação de equipamento",
      "Bridões artesanais", "Acessórios em couro", "Gravações personalizadas",
    ...
    premios: [
      { titulo: "Prémio Nacional de Artesanato", ano: 2019, entidade: "IEFP" }
    ],
    seguroProfissional: true,
    destaque: true,
    disponivel: true,
  ...
  },
  {
    ...
    descricao: "Tosquiadora profissional certificada com especialização em tosquia de competição e artística. Técnicas gentis que minimizam stress. Equipamento silencioso de última geração.",
    avaliacao: 4.8,
    numAvaliacoes: 112,
    servicos: [
      "Tosquia completa", "Tosquia de competição", "Tosquia artística",
      "Trace clip", "Blanket clip", "Hunter clip", "Aparar crinas e caudas",
    ...
    credenciais: [
      "Certificação em Grooming Equino - UK",
      "Formação em Handling Gentil"
    ],
    metricas: {
      tempoResposta: "menos de 4 horas",
      taxaSatisfacao: 96,
  ...
  },
  {
    ...
    descricao: "Fisioterapeuta equina certificada com formação no Reino Unido. Tratamento de lesões musculoesqueléticas, reabilitação pós-cirúrgica e manutenção de cavalos de desporto.",
    avaliacao: 4.9,
    numAvaliacoes: 87,
    servicos: [
      "Fisioterapia manual", "Massagem desportiva", "Reabilitação pós-lesão",
      "Eletroterapia", "Laser terapia", "Kinesiotaping equino",
      "Hidroterapia", "Programa de exercícios"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 9,
    especializacoes: [
      { nome: "Fisioterapia Equina", nivel: "especialista", certificado: "ACPAT Registered", anoObtencao: 2017, instituicao: "Association of Chartered Physiotherapists in Animal Therapy" },
  ...
  },
  {
    ...
    descricao: "Veterinário especializado em medicina desportiva equina com foco em otimização de performance. Testes de esforço, análise de lactato, monitorização cardíaca. Veterinário oficial de eventos FEI.",
    avaliacao: 4.8,
    numAvaliacoes: 134,
    servicos: [
      "Testes de performance", "Análise de lactato", "Monitorização cardíaca",
      "Avaliação pré-competição", "Tratamento de lesões desportivas",
      "Consultoria de treino", "Recuperação pós-competição"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 14,
    especializacoes: [
      { nome: "Medicina Desportiva", nivel: "especialista", certificado: "FEI Official Veterinarian", anoObtencao: 2015 },
  ...
  },
  {
    ...
    descricao: "Ex-cavaleiro internacional de saltos com participações em Taças das Nações. Formador de jovens talentos com múltiplos alunos na seleção nacional. Especialista em formação de cavalos jovens para salto.",
    avaliacao: 4.8,
    numAvaliacoes: 98,
    servicos: [
      "Treino de salto", "Formação de cavalos jovens", "Preparação para competição",
      "Clínicas de salto", "Avaliação de potencial", "Consultoria de compra"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 20,
    especializacoes: [
      { nome: "Salto de Obstáculos", nivel: "especialista", certificado: "FEP Nível III", anoObtencao: 2010 },
  ...
  },
  {
    ...
    descricao: "Especialista em medicina interna equina com foco em gastroenterologia e doenças metabólicas. Referência nacional no tratamento de cólicas e síndrome metabólica equina.",
    avaliacao: 4.9,
    numAvaliacoes: 156,
    servicos: [
      "Medicina interna", "Diagnóstico de cólica", "Tratamento de úlceras",
      "Síndrome metabólica", "Endocrinologia", "Doenças hepáticas",
      "Gastroscopia", "Segunda opinião"
    ],
    nivelVerificacao: "expert",
    experienciaAnos: 16,
    especializacoes: [
      { nome: "Medicina Interna", nivel: "especialista", certificado: "ECEIM Diplomate", anoObtencao: 2014, instituicao: "European College of Equine Internal Medicine" },
    ...
    publicacoes: [
      ...
      { titulo: "Equine Metabolic Syndrome in Lusitano Horses", revista: "Equine Veterinary Journal", ano: 2023, tipo: "artigo" }
    ],
    seguroProfissional: true,
    destaque: true,
    disponivel: true,
  ...
  },
  {
    ...
    descricao: "Treinador especializado em atrelagem com participações em campeonatos mundiais. Formação de cavalos e cocheiros desde iniciação até competição internacional. Preservação da atrelagem tradicional portuguesa.",
    avaliacao: 4.9,
    numAvaliacoes: 45,
    servicos: [
      "Treino de atrelagem", "Formação de cavalos", "Preparação para competição",
      "Atrelagem tradicional", "Clínicas especializadas", "Consultoria de equipamento"
    ],
    nivelVerificacao: "certificado",
    experienciaAnos: 25,
    especializacoes: [
      { nome: "Atrelagem Desportiva", nivel: "especialista", certificado: "FEI Level 2", anoObtencao: 2012 },
    ...
    associacoes: ["FEI", "FEP", "APSL"],
    premios: [
      { titulo: "Medalha de Bronze - Campeonato Mundial", ano: 2018, entidade: "FEI" }
    ],
    seguroProfissional: true,
    disponivel: true,
...
const eventosDB: Evento[] = [
  { id: "1", titulo: "Clínica de Dressage com Maria Santos", tipo: "clinica", data: "2024-05-20", local: "Cascais", organizador: "Maria Santos", preco: "€300", vagas: 8, descricao: "Clínica intensiva de 2 dias para cavaleiros de todos os níveis" },
  { id: "2", titulo: "Workshop de Ferração Ortopédica", tipo: "workshop", data: "2024-06-01", local: "Golegã", organizador: "Manuel Ferreira", preco: "€600", vagas: 6, descricao: "Formação prática em ferração ortopédica e casos complexos" },
  { id: "3", titulo: "Conferência Anual APSL 2024", tipo: "conferencia", data: "2024-07-15", local: "Golegã", organizador: "APSL", preco: "€50", vagas: 200, descricao: "Encontro anual de criadores com palestras e apresentações" },
  { id: "4", titulo: "Webinar: Nutrição do Cavalo de Desporto", tipo: "webinar", data: "2024-05-10", local: "Online", organizador: "Dr. Miguel Fernandes", preco: "Gratuito", vagas: 100, descricao: "Webinar sobre nutrição optimizada para cavalos de competição" },
  { id: "5", titulo: "Curso de Primeiros Socorros Equinos", tipo: "curso", data: "2024-04-20", local: "Lisboa", organizador: "Dr. António Silva", preco: "€150", vagas: 20, descricao: "Aprenda a lidar com emergências equinas enquanto espera pelo veterinário" }
];

const artigosDB: ArtigoEducativo[] = [
  { id: "1", titulo: "Guia Completo de Exame Pré-Compra", autor: "Dr. António Silva", categoria: "Veterinária", resumo: "O que esperar e exigir num exame pré-compra profissional", data: "2024-03", leituras: 3456 },
  { id: "2", titulo: "A Importância da Ferração Correcta", autor: "Manuel Ferreira", categoria: "Ferração", resumo: "Como uma boa ferração impacta a performance e longevidade do cavalo", data: "2024-02", leituras: 2134 },
  { id: "3", titulo: "Do Desbaste ao Grand Prix: Uma Jornada", autor: "Maria Santos", categoria: "Treino", resumo: "A metodologia de formação de cavalos jovens para o dressage de alta competição", data: "2024-01", leituras: 4521 },
  { id: "4", titulo: "Reprodução Equina Moderna", autor: "Dra. Carla Mendes", categoria: "Reprodução", resumo: "Avanços em ICSI e transferência de embriões no cavalo Lusitano", data: "2024-03", leituras: 1876 }
];
...
function BarraExpertise({ nivel }: { nivel: NivelExpertise }) {
  ...
  return (
    <div className="flex gap-1">
      {niveis.map((n, i) => (
        <div key={n} className={`h-1.5 w-4 rounded-full ${i <= index ? config.cor : 'bg-zinc-700'}`} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div className="bg-gradient-to-br from-green-500/20 to-zinc-900 border border-green-500/20 rounded-xl p-4 text-center">
        ...
        <div className="text-2xl font-bold text-white">{stats.casosResolvidos.toLocaleString()}</div>
  ...
  return (
    <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden hover:border-[#C5A059]/30 transition-all group">
      ...
      <div className="border-t border-zinc-800 p-3 flex gap-2">
        <button onClick={onClick} className="flex-1 py-2 bg-[#C5A059] rounded-lg text-xs text-black font-medium hover:bg-[#D4AF6A] transition-colors flex items-center justify-center gap-1">
          Ver Perfil <ChevronRight size={14} />
        </button>
        <a href={`tel:${prof.telefone}`} className="p-2 bg-zinc-800 rounded-lg text-zinc-300 hover:bg-zinc-700 transition-colors">
          <Phone size={14} />
        </a>
      </div>
    </div>
...
function ModalProfissional({ profissional, onClose }: { profissional: Profissional; onClose: () => void }) {
  const [aba, setAba] = useState<"info" | "especializacoes" | "testemunhos" | "disponibilidade" | "formacao">("info");

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-sm overflow-y-auto" onClick={onClose}>
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-3xl w-full max-h-[95vh] overflow-y-auto my-4" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-2 flex-wrap">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-zinc-500">({profissional.numAvaliacoes} avaliações)</span>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {profissional.servicos.map((s, i) => (
                    <span key={i} className="px-3 py-1 bg-zinc-800 rounded-full text-xs text-zinc-300">{s}</span>
                  ))}
                </div>
              </div>
              <div>
              ...
              {profissional.redesSociais && (
                <div className="flex gap-2">
                  {profissional.redesSociais.instagram && <a href="#" className="p-2 bg-zinc-800 rounded-lg text-zinc-400 hover:text-pink-400"><Instagram size={16} /></a>}
                  {profissional.redesSociais.facebook && <a href="#" className="p-2 bg-zinc-800 rounded-lg text-zinc-400 hover:text-blue-400"><Facebook size={16} /></a>}
                  {profissional.redesSociais.youtube && <a href="#" className="p-2 bg-zinc-800 rounded-lg text-zinc-400 hover:text-red-400"><Youtube size={16} /></a>}
                  {profissional.redesSociais.linkedin && <a href="#" className="p-2 bg-zinc-800 rounded-lg text-zinc-400 hover:text-blue-500"><Linkedin size={16} /></a>}
                </div>
              )}
            </div>
          )}
          ...
          {aba === "especializacoes" && (
            <div className="space-y-4">
              {profissional.especializacoes.map((esp, i) => (
                <div key={i} className="bg-zinc-800/30 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs px-2 py-1 rounded ${EXPERTISE_CONFIG[esp.nivel].cor} text-white`}>{EXPERTISE_CONFIG[esp.nivel].label}</span>
                  </div>
              ...
              {profissional.premios && profissional.premios.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-sm font-semibold text-[#C5A059] mb-3 flex items-center gap-2"><Trophy size={16} />Prémios</h3>
                  {profissional.premios.map((p, i) => (
                    <div key={i} className="bg-[#C5A059]/10 border border-[#C5A059]/20 rounded-lg p-3 mb-2">
                      ...
                      <div className="text-xs text-zinc-400">{p.entidade} • {p.ano}</div>
                    </div>
                  ))}
                </div>
              )}
              ...
              {profissional.testemunhos && profissional.testemunhos.length > 0 ? (
                <>
                  {profissional.testemunhos.map((t, i) => (
                    <div key={i} className="bg-zinc-800/30 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="flex">{[...Array(5)].map((_, j) => (<Star key={j} size={12} className={j < t.avaliacao ? "text-[#C5A059] fill-[#C5A059]" : "text-zinc-600"} />))}</div>
                  ...
                  {profissional.casosSucesso && profissional.casosSucesso.length > 0 && (
                    <div className="mt-6">
                      <h3 className="text-sm font-semibold text-[#C5A059] mb-3 flex items-center gap-2"><TrendingUp size={16} />Casos de Sucesso</h3>
                      {profissional.casosSucesso.map((c, i) => (
                        <div key={i} className={`rounded-lg p-4 mb-3 ${c.destaque ? 'bg-[#C5A059]/10 border border-[#C5A059]/20' : 'bg-zinc-800/30'}`}>
          ...
          {aba === "disponibilidade" && (
            <div className="space-y-6">
              <div>
                ...
                <div className="bg-zinc-800/30 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {profissional.disponibilidade.emergencias24h && (<span className="px-2 py-1 bg-red-500/20 text-red-400 rounded text-xs flex items-center gap-1"><Siren size={10} />Emergências 24h</span>)}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {["Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado", "Domingo"].map((d) => (
                      <span key={d} className={`px-3 py-1 rounded text-xs ${profissional.disponibilidade.diasSemana.includes(d) ? "bg-[#C5A059]/20 text-[#C5A059] border border-[#C5A059]/30" : "bg-zinc-800 text-zinc-500"}`}>{d.substring(0, 3)}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#C5A059]/10 border border-[#C5A059]/20 rounded-lg p-4">
                ...
                <div className="flex gap-2">
                  <a href={`tel:${profissional.telefone}`} className="flex-1 flex items-center justify-center gap-2 py-2 bg-[#C5A059] rounded-lg text-black font-medium hover:bg-[#D4AF6A] transition-colors"><Phone size={16} />Ligar</a>
                  <a href={`mailto:${profissional.email}`} className="flex-1 flex items-center justify-center gap-2 py-2 bg-zinc-800 rounded-lg text-zinc-300 hover:bg-zinc-700 transition-colors"><Mail size={16} />Email</a>
                </div>
              </div>
            </div>
          )}
          ...
          {aba === "formacao" && (
            <div className="space-y-4">
                ...
                <>
                  ...
                  {profissional.formacao.map((f, i) => (
                    <div key={i} className="bg-zinc-800/30 rounded-lg p-4 flex items-start gap-3">
                      <div className="p-2 bg-[#C5A059]/20 rounded-lg"><GraduationCap size={16} className="text-[#C5A059]" /></div>
                      <div>
              ...
              {profissional.publicacoes && profissional.publicacoes.length > 0 && (
                <div className="mt-6">
                  ...
                  {profissional.publicacoes.map((p, i) => (
                    <div key={i} className="bg-zinc-800/30 rounded-lg p-4 flex items-start gap-3">
                      <div className="p-2 bg-blue-500/20 rounded-lg">{p.tipo === 'video' ? <PlayCircle size={16} className="text-blue-400" /> : <FileText size={16} className="text-blue-400" />}</div>
                      <div>
                        <div className="font-medium text-white text-sm">{p.titulo}</div>
              ...
              {profissional.cursosOferecidos && profissional.cursosOferecidos.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-sm font-semibold text-[#C5A059] mb-3 flex items-center gap-2"><BookOpen size={16} />Cursos Oferecidos</h3>
  ...
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
      {eventos.slice(0, 3).map((e) => (
        <div key={e.id} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 hover:border-[#C5A059]/30 transition-colors">
          <div className="flex items-center justify-between mb-3">
            <span className={`px-2 py-1 rounded text-xs ${tipoConfig[e.tipo].bg} ${tipoConfig[e.tipo].cor}`}>{e.tipo}</span>
...
export default function ProfissionaisPage() {
  ...
  const [abaAtiva, setAbaAtiva] = useState<"profissionais" | "eventos" | "artigos">("profissionais");

  const stats = useMemo(() => calcularEstatisticas(), []);

  const profissionaisFiltrados = useMemo(() => {
    return profissionaisDB.filter((p) => {
  ...
  return (
    <main className="min-h-screen bg-black text-white pt-20 pb-32 px-4 sm:px-6 md:px-12">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <Link href="/" className="inline-flex items-center gap-2 text-zinc-400 hover:text-[#C5A059] transition-colors mb-6">
          <ArrowLeft size={18} /><span className="text-sm">Voltar</span>
        </Link>
        <div className="text-center">
          <span className="text-[#C5A059] uppercase tracking-[0.3em] text-[9px] font-bold block mb-2">Comunidade Lusitana</span>
          <h1 className="text-2xl sm:text-4xl font-serif italic mb-4">Rede Profissional</h1>
          <p className="text-zinc-400 text-sm max-w-2xl mx-auto">A maior rede de profissionais especializados em cavalos Lusitanos em Portugal. Veterinários, ferradores, treinadores e mais.</p>
        </div>
      </div>
        ...
        <>
          ...
          <div className="max-w-7xl mx-auto mb-6">
            <div className="flex gap-3 flex-wrap">
              ...
              <select value={distritoAtivo} onChange={(e) => setDistritoAtivo(e.target.value)}
              ...
              <select value={filtroVerificacao} onChange={(e) => setFiltroVerificacao(e.target.value as typeof filtroVerificacao)}
                className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#C5A059]/50">
            ...
            <div className="mt-3 text-xs text-zinc-500">{profissionaisFiltrados.length} profissionais encontrados</div>
          </div>
            ...
            {profissionaisFiltrados.length > 0 ? (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {profissionaisFiltrados.map((prof) => (
                  <CardProfissional key={prof.id} prof={prof} onClick={() => setProfissionalSelecionado(prof)} />
                ))}
              </div>
            ) : (
      ...
      {abaAtiva === "artigos" && (
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {artigosDB.map((a) => (
              <div key={a.id} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 hover:border-[#C5A059]/30 transition-colors">
        ...
        <div className="bg-gradient-to-r from-[#C5A059]/10 to-transparent border border-[#C5A059]/20 rounded-xl p-6">
          <div className="sm:flex sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-400">Junte-se à nossa rede verificada e alcance milhares de criadores e proprietários.</p>
            </div>
            <button className="mt-4 sm:mt-0 px-6 py-3 bg-[#C5A059] text-black font-medium rounded-lg hover:bg-[#D4AF6A] transition-colors flex items-center gap-2">
              <ShieldCheck size={18} />Registar-se
            </button>
          </div>
        </div>
      </div>
      ...
      {profissionalSelecionado && (
        <ModalProfissional profissional={profissionalSelecionado} onClose={() => setProfissionalSelecionado(null)} />
      )}
    </main>
  );
}
